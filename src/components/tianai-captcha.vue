<script setup lang="ts">

import {Ref, ref, watch, computed, watchEffect} from "vue";
import axios from "axios";

const props = defineProps({
  captcha_data: {
    type: Object,
    required: true
  },
  captcha_valid: {
    type: Boolean,
    required: true
  },
  refresh_captcha: {
    type: Boolean,
    required: true
  },
})

const emits = defineEmits(['changeValid', 'changeRefresh', 'changeData'])

interface Track {
  x: number
  y: number
  type: "down" | "move" | "up" | "click"
  t: number
}

interface ValidCaptchaData {
  bgImageHeight: number
  bgImageWidth: number
  endSlidingTime: Date
  startSlidingTime: Date
  sliderImageHeight: number
  sliderImageWidth: number
  trackList: Track[]
}

interface ValidCaptcha {
  id: string
  data: ValidCaptchaData
}

interface ClickPosition {
  left: number
  top: number
}

const click_positions: Ref<ClickPosition[]> = ref([])
const slider_move_btn: Ref<HTMLElement | null> = ref(null)
const word_click_elem: Ref<HTMLElement | null> = ref(null)
const is_moving = ref(false)
const offsetX = ref(0)
const rotateDeg = ref(0)
const startX = ref(0)
const startY = ref(0)
const randomY = ref(0)
const startTime = ref(0)
const endTime = ref(0)
const duration = computed(() => {
  return (endTime.value - startTime.value) / 1000
})
const has_valid = ref(false)
const is_valid = ref(false)
const expired_valid = ref(false)
const close = ref(false)
const valid_data = ref({} as ValidCaptcha)
const tracks: Ref<Track[]> = ref([])

const current_type: Ref<"SLIDER" | "CONCAT" | "ROTATE" | "WORD_IMAGE_CLICK" | undefined> = ref()
const tian_main_class = ref("")
const background_position = ref("")
const slider_rotate_offsetX = ref(0)

function check_captcha() {
  valid_data.value.data.trackList = tracks.value
  valid_data.value.data.startSlidingTime = new Date(startTime.value)
  valid_data.value.data.endSlidingTime = new Date(endTime.value)
  axios.create({
    withCredentials: true
  }).post('api/captcha/check', valid_data.value).then(value => {
    is_valid.value = value.data.success
    has_valid.value = true
    expired_valid.value = value.data.code === 4000
    new Promise((resolve) => setTimeout(resolve, 1000))
        .then(() => {
          emits('changeRefresh', !is_valid.value)
          emits('changeValid', is_valid.value)
        })
  })
}

function addTrack(e: MouseEvent) {
  tracks.value.push({
    type: e.type.replace('mouse', ''),
    x: e.pageX - startX.value,
    y: e.pageY - startY.value,
    t: (new Date().getTime() - startTime.value)
  } as Track)
}

function captcha_mousedown(e: MouseEvent) {
  is_moving.value = true
  startX.value = e.pageX
  startY.value = e.pageY

  addTrack(e)
}

function captcha_mousemove(e: MouseEvent) {
  if (is_moving.value === true) {
    offsetX.value = e.pageX - startX.value
    if (offsetX.value < 0) offsetX.value = 0
    if (offsetX.value > 242) offsetX.value = 242
    rotateDeg.value = current_type.value === "ROTATE" ? offsetX.value / 242 * 360 : 0
    slider_rotate_offsetX.value = current_type.value === "ROTATE" ? 0 : offsetX.value

    addTrack(e)
  }
}

function captcha_mouseup(e: MouseEvent) {
  is_moving.value = false
  endTime.value = new Date().getTime()
  addTrack(e)
  check_captcha()
}

function captcha_mouseclick(e: MouseEvent) {
  endTime.value = new Date().getTime()
  addTrack(e)

  click_positions.value.push(
      {
        left: e.offsetX - 10,
        top: e.offsetY - 10
      } as ClickPosition
  )

  if (click_positions.value.length == 4) {
    check_captcha()
  }

}

function init_captcha() {
  valid_data.value = {
    id: props.captcha_data.id,
    data: {
      bgImageHeight: props.captcha_data.captcha.backgroundImageHeight / 2,
      bgImageWidth: props.captcha_data.captcha.backgroundImageWidth / 2,
      sliderImageHeight: props.captcha_data.captcha.templateImageHeight / 2,
      sliderImageWidth: props.captcha_data.captcha.templateImageWidth / 2,
    }
  } as ValidCaptcha
  tracks.value = []
  click_positions.value = []
  offsetX.value = 0
  slider_rotate_offsetX.value = 0
  startX.value = 0
  startY.value = 0
  rotateDeg.value = 0
  randomY.value = 0
  startTime.value = new Date().getTime()
  endTime.value = 0

  has_valid.value = false
  expired_valid.value = false
  is_moving.value = false

  tian_main_class.value = ""
  background_position.value = ""

  if (props.captcha_data.captcha.data) {
    randomY.value = (360 - props.captcha_data.captcha.data.randomY) / 2
  }
  current_type.value = props.captcha_data.captcha.type

  switch (current_type.value) {
    case "SLIDER": {
      tian_main_class.value = "tianai-captcha-slider"
      break
    }
    case "CONCAT": {
      tian_main_class.value = "tianai-captcha-slider tianai-captcha-concat"
      break
    }
    case "ROTATE": {
      tian_main_class.value = "tianai-captcha-slider tianai-captcha-rotate"
      background_position.value = "-5px 31.0092%"
      valid_data.value.data.bgImageWidth = 242
      break
    }
    case "WORD_IMAGE_CLICK": {
      tian_main_class.value = "tianai-captcha-slider tianai-captcha-word-click"
      is_moving.value = true
    }
  }

}

watchEffect(() => {
  if (word_click_elem.value === null) return
  if (slider_move_btn.value) {
    // not word image click
    slider_move_btn.value.addEventListener('mousemove', captcha_mousemove)
    slider_move_btn.value.addEventListener('mousedown', captcha_mousedown)
    slider_move_btn.value.addEventListener('mouseup', captcha_mouseup)

    word_click_elem.value.removeEventListener('mousemove', captcha_mousemove)
    word_click_elem.value.removeEventListener('click', captcha_mouseclick)
  } else {
    // word image click
    word_click_elem.value.addEventListener('mousemove', captcha_mousemove)
    word_click_elem.value.addEventListener('click', captcha_mouseclick)
  }
})

watch(() => props.captcha_data, () => {
  init_captcha()
})

</script>

<template>
  <div id="captcha-box" v-if="!close">
    <div id="tianai-captcha-parent">
      <div id="tianai-captcha-bg-img"></div>
      <div id="tianai-captcha-box">
        <img class="loading" style="display: none"
             src="data:image/gif;base64,"
             alt="loading"/>
        <div id="tianai-captcha" :class="tian_main_class" style="transform: translateX(0px)">

          <div class="slider-tip" v-if="current_type !== 'WORD_IMAGE_CLICK'">
            <span id="tianai-captcha-slider-move-track-font">拖动滑块完成拼图</span>
          </div>

          <div class="click-tip" v-else>
            <span id="tianai-captcha-click-track-font">请依次点击:</span>
            <img :src="captcha_data.captcha.templateImage" alt="" class="tip-img"/>
          </div>

          <div class="content" ref="word_click_elem">
            <div class="bg-img-div" v-if="current_type !== 'CONCAT'">
              <!--背景图片-->
              <img id="tianai-captcha-slider-bg-img" :src="captcha_data.captcha.backgroundImage" alt=""/>
              <canvas id="tianai-captcha-slider-bg-canvas"/>
              <div id="bg-img-click-mask" v-if="current_type === 'WORD_IMAGE_CLICK'">
              <span class="click-span" v-for="(pos, index) in click_positions"
                    :style="{left: `${pos.left}px`, top: `${pos.top}px`}">{{ index + 1 }}</span>
              </div>
            </div>
            <div v-else>
              <div class="tianai-captcha-slider-concat-img-div" id="tianai-captcha-slider-concat-img-div"
                   :style="{ height: `${randomY}px`, backgroundPosition: `${offsetX}px 0px`, backgroundImage: `url(&quot;${captcha_data.captcha.backgroundImage}&quot;)` }">
                <!--背景图片-->
                <img id="tianai-captcha-slider-concat-slider-img" src="" alt=""/>
              </div>
              <div class="tianai-captcha-slider-concat-bg-img" id="tianai-captcha-slider-img-div"
                   :style="{backgroundImage: `url(&quot;${captcha_data.captcha.backgroundImage}&quot;)`}">
                <!--移动的图片-->
              </div>
            </div>

            <div :class="current_type.toLowerCase() + '-img-div'" id="tianai-captcha-slider-img-div"
                 :style="{ transform: `rotate(${rotateDeg}deg) translateX(${slider_rotate_offsetX}px)` }"
                 v-if="current_type === 'SLIDER' || current_type === 'ROTATE'"
            >
              <!--移动的图片-->
              <img id="tianai-captcha-slider-move-img" :src="captcha_data.captcha.templateImage" alt=""/>
            </div>

            <div class="tianai-captcha-tips tianai-captcha-tips-error tianai-captcha-tips-on"
                 id="tianai-captcha-tips" v-if="has_valid && !is_valid && !expired_valid">
              验证失败，请重新尝试
            </div>
            <div class="tianai-captcha-tips tianai-captcha-tips-error tianai-captcha-tips-on"
                 id="tianai-captcha-tips" v-if="has_valid && expired_valid">
              验证码被黑洞吸走了
            </div>
            <div class="tianai-captcha-tips tianai-captcha-tips-success tianai-captcha-tips-on"
                 id="tianai-captcha-tips" v-if="has_valid && is_valid">
              验证成功,耗时 {{ duration }} 秒
            </div>
          </div>
          <div class="slider-move" v-if="current_type !== 'WORD_IMAGE_CLICK'">
            <div class="slider-move-track">
              <div id="tianai-captcha-slider-move-track-mask"
                   style="border-color: rgb(2, 152, 248);background-color: rgb(137,210,255)"></div>
              <div class="slider-move-shadow"></div>
            </div>
            <div class="slider-move-btn" id="tianai-captcha-slider-move-btn" ref="slider_move_btn"
                 :style="{ transform: `translateX(${offsetX}px)`, backgroundPosition: background_position }"></div>
          </div>
        </div>
      </div>

      <div class="slider-bottom">
        <img class="logo" id="tianai-captcha-logo" src="@/assets/images/logo.png" alt=""/>
        <div class="close-btn" id="tianai-captcha-slider-close-btn" @click="close = true"></div>
        <div class="refresh-btn" id="tianai-captcha-slider-refresh-btn" @click="emits('changeRefresh', true)"></div>
      </div>
    </div>

  </div>
</template>

<style scoped>

</style>